...
export default class World {
  ...
  addEntity(entity: Entity) {
    for (const key in entity._components) {
      ...
      this._familiesIndex[component.name] =
        this._familiesIndex[component.name] || [];
      const families = this._familiesIndex[component.name];
  ...
  getEntities(...componentNames: string[]): Array<any>[] {
    ...
    const familyId = this._getFamilyId(componentNames);
  ...
  entityAdded(componentNames: string | string[]): Signal {
    ...
    const familyId = this._getFamilyId(components);
  ...
  private _onComponentAdded(entity: Entity, componentName: string) {
    this._familiesIndex[componentName] =
      this._familiesIndex[componentName] || [];
    const families = this._familiesIndex[componentName];
